...
exports.default = {
  install: function install(Vue, connection, store) {
    var _this = this;

    if (!connection) throw new Error("[Vue-Socket.io] cannot locate connection");

    var observer = new _Observer2.default(connection, store);

    if ((typeof connection === 'undefined' ? 'undefined' : _typeof(connection)) === 'object') {
      Vue.prototype.$socket = [];
      Object.keys(connection).forEach(function (key) {
        var socket = Vue.prototype.$socket[key] = observer.Sockets[key];
        Vue.prototype.$socket[key].$on = function (evt, cb) {
          console.log('adding emitter');
          _Emitter2.default.addListener(evt, cb, _this);
        };
        var super_onevent = socket.onevent;
        socket.onevent = function (packet) {
          super_onevent.call(socket, packet);
          console.log('unknown pkt2', key, packet.data[0], packet.data[1]);
          _Emitter2.default.emit(packet.data[0], packet.data[1]);
          if (store) observer.passToStoreWithKey.apply(observer, [key.toUpperCase() + '_SOCKET_' + packet.data[0]].concat(_toConsumableArray(packet.data.slice(1)), [key]));
        };
        // Vue.prototype.$socket[key].on() = observer.Sockets[key];
      });
    } else {
    ...
    // Vue.mixin({
    //     methods: {
    //         onsocketreceived (data) {
    //             console.log('hello from mixin!', data)
    //         }
    //     }
    // })
  }
...
var _class = function () {
    function _class(connection, store) {
        var _this2 = this;
        ...
        } else if ((typeof connection === 'undefined' ? 'undefined' : _typeof(connection)) === 'object') {
            this.Sockets = [];
            Object.keys(connection).forEach(function (key) {
                _this2.Sockets[key] = (0, _socket2.default)(connection[key]);
            });
        } else {
    ...
    _createClass(_class, [{
        ...
        key: 'onEventWithKey',
        value: function onEventWithKey(socket, key) {
            var _this = this;
            // let super_onevent = socket.onevent;
            // socket.onevent = (packet) => {
            //     super_onevent.call(socket, packet);
            //     console.log('unknown pkt', key, packet.data[0], packet.data[1])
            ...
            ["connect", "error", "disconnect", "reconnect", "reconnect_attempt", "reconnecting", "reconnect_error", "reconnect_failed", "connect_error", "connect_timeout", "connecting", "ping", "pong"].forEach(function (value) {
                socket.on(value, function (data) {
                    console.log('socket has', key, value, data);
        ...
        key: 'passToStoreWithKey',
        value: function passToStoreWithKey(event, payload, key) {
            if (!event.startsWith(key.toUpperCase() + '_SOCKET_')) return;
            for (var namespaced in this.store._mutations) {
                var mutation = namespaced.split('/').pop();
                if (mutation === event.toUpperCase()) this.store.commit(namespaced, payload);
            }

            for (var _namespaced in this.store._actions) {
                var action = _namespaced.split('/').pop();
                if (!action.startsWith(key + '_socket_')) continue;

                var camelcased = key + '_socket_' + event.replace(key.toUpperCase() + '_SOCKET_', '').replace(/^([A-Z])|[\W\s_]+(\w)/g, function (match, p1, p2) {
                    return p2 ? p2.toUpperCase() : p1.toLowerCase();
                });

                if (action === camelcased) this.store.dispatch(_namespaced, payload);
            }
        }
    }, {
        key: 'onEvent',
        value: function onEvent() {
            ...
            this.Socket.onevent = function (packet) {
                super_onevent.call(_this3.Socket, packet);

                _Emitter2.default.emit(packet.data[0], packet.data[1]);

                if (_this3.store) _this3.passToStore.apply(_this3, ['SOCKET_' + packet.data[0]].concat(_toConsumableArray(packet.data.slice(1))));
            };
        ...
        key: 'passToStore',
        value: function passToStore(event, payload) {
            ...
            for (var _namespaced2 in this.store._actions) {
                var action = _namespaced2.split('/').pop();

                if (!action.startsWith('socket_')) continue;

                var camelcased = 'socket_' + event.replace('SOCKET_', '').replace(/^([A-Z])|[\W\s_]+(\w)/g, function (match, p1, p2) {
                    return p2 ? p2.toUpperCase() : p1.toLowerCase();
                });

                if (action === camelcased) this.store.dispatch(_namespaced2, payload);
            }
        }
    }]);
